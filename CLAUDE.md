...
>
> **다음에 뭘 할 차례인지는 `docs/CODEX-다음작업.md` 에 있다.** 시작할 때 읽고, 끝낼 때 갱신한다.
>
> 마지막 갱신: 2026-09-22
...
| 문서 | 기준 |
|---|---|
| `docs/CODEX-다음작업.md` | ✅ **다음 작업 · 지금 상태** (Codex 와 공용. 작업 끝낼 때 갱신) |
...
| 로봇 이동 API 규격 | 시뮬레이션팀 |
| 기관 출결 API 규격 | 기관 |
| 영수증 프린터 **기종** | 기관 (용지 폭은 80mm/58mm 전환 지원. 기종·해상도는 아직 미정) |

가짜 장소명이나 그럴듯한 API 스펙을 **만들어 넣지 말 것.** 자리표시자로 두고 사용자에게 알린다.

## ⚠️ 9. 화면 디자인은 동결이다

시연 자료와 팀 설명이 지금 화면 기준으로 만들어져 있다. 화면이 바뀌면 설명이 전부 어긋난다.

- 구성·흐름·색·여백·모서리·글꼴 체계를 **임의로 바꾸지 않는다.**
- 새 화면이 필요하면 `web/src/styles.css` 의 **기존 클래스를 재사용**해 같은 모양으로 만든다.
- 고쳐도 되는 것: 버그, 기능 추가, 데이터 연결, 접근성·가독성 보정.
- 리디자인이 필요하다고 판단되면 **먼저 사용자에게 묻는다.**

### 터치 화면(패드·휴대폰)

키오스크 하드웨어가 태블릿일 가능성이 높다. `styles.css` 끝부분에 대응 규칙이 모여 있다.

- 화면 CSS 를 고쳤으면 **360 ~ 1366px 폭에서 가로 스크롤이 생기지 않는지** 확인한다.
  장식 원(`.orbit`)처럼 `position:absolute` 인 요소는 부모의 `transform:scale()` 로 줄여도
  **레이아웃 상자는 그대로**라서 조용히 화면 밖으로 삐져나간다.
- 입력칸 글씨는 좁은 화면에서 **16px 이상**이어야 한다. iOS 사파리는 그보다 작으면 커서를 넣을 때
  화면을 확대하고, 확대된 키오스크 화면은 어르신이 되돌리지 못한다.
- 폭 600px 이하에서는 로봇 일러스트를 숨기고 본문을 전체 폭으로 쓴다.
- 폭 900px 이하에서는 직원 화면의 왼쪽 메뉴가 위쪽 가로 메뉴로 바뀐다.

### 글씨 굵기

주 이용자가 고령층이라 본문을 한 단계 굵게 쓴다(`:root` 500, 키오스크 본문 600).
**윈도우 기본 글꼴(맑은 고딕)은 400·700 두 굵기뿐이라 500 은 400 으로 떨어진다.**
`:root` 에 `font-synthesis:none` 이 있어 가짜 굵게도 만들지 않는다.
윈도우에서 꼭 굵어야 하는 글은 600 이상으로 둔다.
...
> 최신 목록은 `docs/CODEX-다음작업.md` 에 있다. 아래는 요약이다.

- ~~장소 검색~~ — 2026-09-17 완료 (큰 검색창, 초성·혼합·띄어쓰기 무시, 결과 없음 시 직원 도움 안내)
- ~~문서 정리~~ — 2026-09-17 완료 (`docs/archive/`)
- ~~태블릿·모바일 터치 화면 대응, 고령층용 글씨 굵기~~ — 2026-09-22 완료 (§9)

1. **예약 이동** — 계획서 5번 "프로그램 시작 전 해당 강의실 앞으로 이동".
   ⏸ 시뮬레이션팀의 **로봇 이동 API 규격**이 와야 한다 (§8). 규격 없이 요청/응답 형식을 지어내지 말 것.
2. ~~영수증 인쇄 다듬기~~ — 2026-09-17 완료. 용지 폭 80mm(기본)/58mm 전환, 상세 화면에서 선택.
   규격은 `web/src/receipt.ts` 에 순수 함수로 두고 `receipt.test.ts` 로 검증한다.
   ⏸ 남은 것은 **기종 확정 후 실물 출력 확인**뿐이다 (여백·잘림은 실제로 뽑아봐야 안다).
