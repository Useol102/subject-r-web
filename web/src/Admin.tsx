...
import { day, floorName, request, states, time, type Catalog, type Place, type Program, type Session, type Snapshot, type Trip } from './api'
...
export default function Admin({data,reload,connectionError}:Props){
  ...
  async function save(form:HTMLFormElement){
    ...
    // 프로그램과 회차는 따로 저장한다. 이 폼은 아직 첫 회차만 다룬다 (반복·휴강 편집은 준비 중).
    const {starts_at,ends_at,session_id,...program}=payload as Record<string,unknown>
    const id=session_id?String(session_id):`${String(program.id)}-1`
    const ok=await run(async()=>{
      await request('/admin/programs','PUT',program)
      await request('/admin/sessions','PUT',{id,program_id:program.id,
        starts_at:new Date(String(starts_at)+'+09:00').toISOString(),
        ends_at:new Date(String(ends_at)+'+09:00').toISOString()})
    },'저장했습니다. 이용자 화면에도 반영됩니다.')
...
function Editor({value,places,sessions,busy,onSave}:{value:Place|Program|'new-place'|'new-program';places:Place[];sessions:Session[];busy:boolean;onSave:(form:HTMLFormElement)=>Promise<void>}){
