...
export default function App(){
  ...
  // 시각·장소는 프로그램이 아니라 회차에 있다. 화면은 프로그램 이름과 같이 보여줘야 해서 둘을 묶는다.
  const programById=new Map(programs.map(p=>[p.id,p]))
  const todays=sessions.filter(s=>s.session_day_kst===today).flatMap(s=>{
  ...
  const shownSession=(programId:string)=>{
    ...
    return list.find(s=>s.status==='scheduled'&&s.starts_at>=now.toISOString())||list[list.length-1]
  }
  ...
  async function attend(code:string,target:string){
    ...
    try{const result=await request<{message:string}>('/attendance/demo','POST',{program_session_id:target,code});setAttendanceMessage(result.message)}
  ...
  return <div className={`kiosk ${large?'large-type':''}`}>
    ...
    {page==='detail'&&selected&&<article className="receipt"><h2>Subject R · 안내문</h2><p>{data?.demo?'예시 정보 · 실제 기관 안내가 아닙니다.':''}</p><h1>{pickedProgram?.title||pickedPlace?.name}</h1>{pickedProgram&&shownSession(pickedProgram.id)&&<p>{day(shownSession(pickedProgram.id)!.starts_at)}<br/>{time(shownSession(pickedProgram.id)!.starts_at)} – {time(shownSession(pickedProgram.id)!.ends_at)}</p>}<p>{pickedPlace?`${floorName(pickedPlace.floor)} ${pickedPlace.name}`:''}</p><p>{selected.description}</p><hr/><h3>장소 안내</h3><ol>{pickedPlace?.directions.map((s,i)=><li key={i}>{s}</li>)}</ol><p>자동 경로가 아닌 등록된 안내문입니다.</p><small>궁금한 점은 안내데스크에 물어보세요.</small></article>}
  </div>
}
