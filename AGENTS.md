...
- **작업을 시작하기 전에 `docs/CODEX-다음작업.md` 를 읽는다.** 지금 상태와 다음에 할 일이 거기 있다. 작업을 끝낼 때 그 파일을 갱신해 같은 커밋에 넣는다.
- **화면 디자인은 동결이다.** 구성·색·여백·글꼴 체계를 임의로 바꾸지 않는다. 새 화면이 필요하면 `web/src/styles.css` 의 기존 클래스를 재사용한다. 리디자인은 사용자 승인 후에 한다.
- 이용자는 고령층이다. 본문 글씨는 굵기 500~600 을 쓴다. 윈도우 기본 글꼴(맑은 고딕)은 400·700 뿐이라 500 은 400 으로 떨어진다.
- 키오스크 하드웨어가 태블릿일 수 있다. 화면을 고치면 360~1366px 폭에서 가로 스크롤이 생기지 않는지 확인한다.
