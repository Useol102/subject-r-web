> ⛔ **옛 계획 문서 (보관용).** 2026-09-17 에 프로젝트가 SQLite 기반 키오스크로 전환하면서
> 이 문서는 효력을 잃었다. 현재 규칙은 루트 `AGENTS.md` 와 `CLAUDE.md`, 현재 실행법은
> `docs/WEB-START.md` 를 볼 것. PostgreSQL/PostGIS 설계 근거를 다시 볼 일이 있을 때만 참고한다.
