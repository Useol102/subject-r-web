...
4. `프로그램 관리` 목록의 `회차 N` 버튼으로 회차를 관리한다. 시각은 프로그램이 아니라 **회차**에 있다.
   - `매주 반복으로 추가`에 첫 회차 시각과 횟수를 넣으면 미리보기가 나오고, 저장은 한 트랜잭션으로 들어간다.
     날짜 계산은 `web/src/recurrence.ts` 가 한다.
   - 안 하는 수업은 **지우지 않고 휴강**으로 표시한다(사유 필수). 과거 출결이 회차를 참조하기 때문이다.
     휴강은 이용자 화면에도 `휴강 · 사유`로 보이고 출석 선택지에서는 빠진다.
5. 직원 화면 대시보드에서 장소를 고르고 데모 이동 요청을 만든다.
6. `이동 시작 재현`, `도착 재현` 버튼은 상태 전이 UI만 확인하는 기능이다. 실제 로봇을 움직이지 않는다.
7. `데이터 · 연결`에서 장소·프로그램 JSON을 내보내거나 가져온다. 가져오기는 같은 ID를 수정하고 새 ID를 추가하며, 누락된 데이터는 삭제하지 않는다.

## 화면 크기와 글씨

키오스크 하드웨어가 태블릿일 수 있어서 같은 화면이 패드·휴대폰에서도 그대로 동작한다.

- 폭 900px 이하: 입력칸 글씨가 16px 로 커진다(iOS 사파리가 화면을 확대하지 않게), 직원 화면의 왼쪽 메뉴가 위쪽 가로 메뉴로 바뀐다.
- 폭 600px 이하: 로봇 일러스트를 숨기고 본문이 전체 폭을 쓴다. 안내 카드 3개는 한 줄씩 쌓인다.
- 하단 고정 바는 iOS 홈 인디케이터 영역(`env(safe-area-inset-bottom)`)을 피한다.
- 주 이용자가 고령층이라 본문 굵기를 500~600 으로 올렸다. 윈도우 기본 글꼴(맑은 고딕)은 400·700 뿐이라
  500 은 400 으로 떨어진다. 윈도우에서 꼭 굵어야 하는 글은 600 이상으로 둔다.
- **화면 CSS 를 고치면 360~1366px 폭에서 가로 스크롤이 생기지 않는지 확인한다.** 규칙은 `web/src/styles.css` 끝부분에 모여 있다.

화면 디자인은 동결이다. 구성·색·여백을 임의로 바꾸지 않는다(`CLAUDE.md` §9).
...
- **다음에 무엇을 할 차례인지는 `docs/CODEX-다음작업.md` 에 있다.** 작업을 끝낼 때 그 파일을 갱신한다.
